...
\\The application will be available to the users in web and mobile forms and will have public APIs in order to expand and improve the service with additional features. %TODO ripetuto due volte?
...
\end{comment} %TODO non ho capito davvero cosa sia questa %non dovrebbe essere semplicemente il titolo che raccoglie el successive? in teoria no, è una subsection come le altre
\paragraph{Hardware Interfaces}
\begin{itemize}
\item Owned by the users and taxi drivers:
	\begin{itemize}
		\item Any GPS-capable device running Android 4.0+ or iOS 6+ %TODO in realtà i clienti possono anche non avere il GPS tanto devono inserire a mano la starting location %AH, buono a sapersi, XD non vogliamo almeno facilitare la selezione? E poi ormai il GPS ce l'hanno tutti non è restrittivo (se però puzza lo tolgo, non mi interessa)% mah...io lo toglierei e metterei come domain assumption che i taxi abbiamo un gps nella macchina e basta
...
\begin{itemize}
	...
	\item PHP interpreter:
		...
		\end{itemize}
	\item Operating System: %TODO OS per il server http? %cosa intendi? centos è per entrambi i server va bene sia per dbms che http %aaah ok xD non sapevo cosa fosse
		\begin{itemize}
			\item Name: CentOS
...
%TODO PEGI 18, contiene uso di alcol droga violenza o bestemmie %sono morta XDDDDDDD
...
%TODO mettiamo i gps delle macchine, no?	\item All taxi drivers will have a phone with active GPS functionality
	...
	\item Users will have access to the internet %TODO dici che serve?
	\item In order to help the client to select the current location a phone with GPS capability will be required %TODO non credo sia una domain assumption
	...
	\item The Owner of the app will have to build or rent an always-on DBMS and host for the Server-Side part of the app %TODO ma non era già nei requisiti sopra? non so dove sia più giusto metterlo, per me sta in entrambi i casi % non so se va nei non funct come availability 24/7 del db o qui
	\item Users who have requested or reserved a taxi will always be present when the taxi arrives. %TODO aggiunta
	\item There is always at least an available taxi to fulfil a request or a reservation %TODO aggiunta
\end{itemize}
\clearpage
...
\begin{itemize}
	\item For non logged in users (on both the WEB and Mobile interface):
		\begin{itemize}
			\item The system will be able to calculate the ETA. %TODO modificata
			\item The system will provide a registration functionality %TODO modificata
			\item The system will provide a login functionality
		\end{itemize}
	\item For logged in users (on both the WEB and Mobile interface):
		\begin{itemize}
			\item Provide information about taxis queues and availability %TODO è già scritta sopra di calcolare l'ETA io direi di toglierla, oppure di aggiungere un'altra funzionalità che ti fa vedere la queue, in questo caso devo aggiungerla anche nelle interfacce
			\item The system will store the username, password, email and credit card data of every user %TODO non trovo più dove c'erano scritte le altre cose che devi scrivere nel login e che deve salvare-->sotto ho scritto magari di metterle in un posto comune con altre funct
	...
	\item Through the API:\@ %TODO c'è qualquadra che non cosa, ti va se ne parliamo quando hai tempo?
		\begin{itemize}
			\item Get the queue status for a given position %TODO vedi todo sopra sulle queue
		...
		\end{itemize}
\end{itemize}
On the taxi driver side:\@ %TODO (there will be no Web interface in this case) <- non è meglio specificarlo dove abbiamo detto che interfacce abbiamo?
%TODO i taxi driver sono già considerati loggati per cui non servono le funct di login, piuttosto dobbiamo mettere davvero le funzionalità di reset password? non è meglio mettere una funct di modifica dati personali e basta?
\begin{comment}
\begin{itemize}
...
\end{comment}
	\begin{itemize}
		\item If credentials have been invalidated a ``reset password'' procedure will be mandatory %TODO ?.?
		...
		\item Handle disconnection %TODO da togliere, abbiamo detto come domain assumption che hanno sempre internet
		...
		\item The system will provide a logout functionality
	\end{itemize}
	\item Through the API:\@ %TODO le api mi sono oscure T.T
		\begin{itemize}
			\item Establish a Secure Channel
...
On the admin side:\@
		\begin{itemize}
			...
			\item Set the area of the map handled by the system %TODO davvero? tra l'altro, dove mettiamo le cose che gli input sono suggeriti? nelle funct degli utenti?
...
\begin{itemize}
	\item \textit{Cross platform} %TODO sono un po in dubbio su questo req
		\begin{itemize}
			...
			\item The web application will have to use HTML5 %TODO questo non credo sia un nonfunctional req
		\end{itemize}
	\item \textit{Availability}
		\begin{itemize}
			\item The application must always be available
			\item If a query is taking some time a spinner will be shown, the app must never freeze %TODO questo non è un req di availability
		...
		\end{itemize}
	\item \textit{Usability} %TODO credo che usability non sia un req, nel senso che da noi han detto che è una di quelle cose che dici "ma va?"
		\begin{itemize}
	...
	\item \textit{Security}
		\begin{itemize}
			...
			\item No one should be able to impersonate the taxi drivers, the clients or the admin without proper authentication %TODO mmmhhh questa non mi convince
		\end{itemize}
%	\item 
\end{itemize}
