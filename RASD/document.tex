...
\begin{itemize}
	...
	\item Clients will have access to the internet and a device with GPS integrated %TODO non necessaria
	\item Clients have a valid email address %TODO non necessaria
	...
	\item The Owner of the app will have to build or rent an always-on DBMS and host for the Server-Side part of the app %TODO troppo specifica
...
Donald Duck is ready to start his first day as a taxi driver in Paperopoli. He jumps on his car, logs in TAXINSECONDS and changes his state to available to wait for clients. Unfortunately he doesn't know that in his zone there are 15 taxis the joined the queue before him, so he'll have to be patient for quite some time.
\subsubsection{Scenario 5}
After quite some time, Daisy requests a taxi and it's finally Donald's turn! The system notifies him but...it was breakfast time so Donald's having a cappuccino in a very crowded bar. He doesn't hear the notification popping on his phone so, after 1 minute, the system forwards Daisy's request to the next taxi driver in the queue and changes Donald's state to not available.
