...
%TODO credo di volermi dimenticare del fatto che qualcuno non si presenti dopo aver prenotato il taxi sono cosi' d'accordo XD
...
\item Owned by the company:
	\begin{itemize}
		...
		\item A machine with the DBMS %TODO mi sa che qui è un po troppo implementativo :/ %fixed
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item PHP interpreter:
		...
		\end{itemize}
	\item Operating System: %TODO OS per il server http? %cosa intendi? centos è per entrambi i server va bene sia per dbms che http
		\begin{itemize}
			\item Name: CentOS
...
\begin{itemize}
	...
	\item Secondary memory:
		\begin{itemize}
			...
			\item the web application requires no secondary memory %TODO mi sa che little è troppo vago per i loro gusti
			\item for the server it is suggested to use a cloud service in order to resize memory according to traffic
		\end{itemize}
\end{itemize}
...
\begin{itemize}
	...
	\item The taxi drivers will have to grant the system the rights to handle their taxi codes %TODO lol really? non sapevo servisse xD %fuffa per la privacy, è un dato sensibbbbbbile
	\item Taxi drivers' phones will always have an internet connection while the app is running
	\item Clients will have access to the internet %TODO non necessaria se inserisco starting location a mano
	\item In order to help the client to select the current location a phone with GPS capability will be required
	\item Clients will have a valid email address %TODO non necessaria vedi sotto
	\item Clients will have a credit/debt card %TODO non necessaria, basta che mettiamo un controllo alla registrazione %una delle due serve, come li identifiche se no?
	\item Clients allow the app to access their credit in order to pay for the service
	\item The Owner of the app will have to build or rent an always-on DBMS and host for the Server-Side part of the app %TODO ma non era già nei requisiti sopra? non so dove sia più giusto metterlo, per me sta in entrambi i casi
\end{itemize}
\clearpage
...
\begin{itemize}
	\item For non logged in user (on both the WEB and Mobile interface):
		\begin{itemize}
			\item Provide information about taxis queues and availability %TODO non dovrebbero essere divisi per goal? comunque qua sarei più specifica % tipo? (cmq dove è scritto di dividerli per goals? mi deve essere sfggito)
			\item Registration in order to become a registered user %TODO the system will provide a sign up functionality %registration non ti paice proprio eh?
