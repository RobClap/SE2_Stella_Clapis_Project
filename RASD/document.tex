...
\begin{itemize}
	\item \textit{Secure Channel:} a communication tunnel (TLS) to ensure privacy and autenticity for both the server and the clients %TODO serve davvero?
...
\begin{itemize}
	...
	\item TLS:\@ Transport Layer Security, a cryptographic protocol which is designed to provide communication security over the Internet. %TODO serve davero pt2? non essere troppo specifico nelle scelte di implementazione
\end{itemize}
\subsection{Actors}
\begin{itemize}
...
\begin{itemize}
	...
	\item A lower level interface for APIs %TODO lower?
\end{itemize}
...
\begin{itemize}
	\item Primary memory:
		...
		%TODO Redo this 			\item for the server side 32GB+ are suggested, but with great traffic a bigger memory could be required (5MB per average active user is a minimum size)
	...
	\item Secondary memory:
		...
		%TODO Redo this			\item the server will need to have the storage for the DBMS (with all users data), the application code and system logs, so 1TB of hard drive is required. (50MB for user at least)
		\end{itemize}
\end{itemize}
