...
Mickey Mouse is planning to go fishing with Pluto tomorrow. After logging in TAXINSECONDS, he makes two reservations: one for the morning and one for the evening. %TODO finire
...
\textit{Flow of Events:}
	\begin{itemize}
		...
		%TODO da togliere questo\item  When the taxi driver arrives at the starting location, according to her GPS information, the system automatically makes the user pay the minimum fare 
		\item The taxi driver goes to the starting location to pick up the user
	\end{itemize}
\textit{Postconditions:} The taxi driver is not available anymore and has been removed from the queue. %TODOperchè il taxi non è disponibile? più disponibile di cosi'! sta servendo! %proprio perchè sta servendo non è disponibile ad accettare altre richieste
\\\textit{Exceptions:}
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item  The guest clicks the "Register" button
		\item  The system shows a ''successful registration'' message and brings the guest to the login page
	\end{itemize}
	\textit{Postconditions:} The request has been cancelled.
	\\\textit{Exceptions:}
	\begin{itemize} 
	...
	\subsection{Result}
	%TODO put screenshot of successful build
