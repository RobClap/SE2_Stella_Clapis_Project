...
\newcommand{\comment}[1]{}
\author{Roberto Clapis, Erica Stella} %TODO matricole
\date{\today}
...
\\The city managed by TAXINSECONDS is divided in zones of 2 km² each and every zone has its own queue of taxis. The queues are automatically computed by the system with the information it receives from the GPS of the taxis. 
...
\begin{itemize}
	...
	\item Through the API:\@
		\begin{itemize}
			...
			\item Establish a Secure Channel %TODO add this to definitions 
			...
			\item Using a valid login-token:\@
				\begin{itemize}
					...
					\item Logout (this will close the secure channel and invalidate the login token) %TODO add ``login token'' to definitions 
				\end{itemize}
		\end{itemize}
\end{itemize}
...
\begin{itemize}
	\item \textit{Cross platform}
		\begin{itemize}
			...
			\item At least Android>=4.0 and iOS will be supported for the mobile version %TODO insert a meaningful frocio OS version
			\item At least Edge, Chrome and Firefox will ahve to be supported
			\item The web application will have to use HTML5
		\end{itemize}
	\item \textit{Responsiveness and availability}
		\begin{itemize}
			\item The application must always respond, even if no taxi are available
	...
	\item \textit{Security}
		\begin{itemize}
			...
			\item 
		\end{itemize}
	\item The service must be always
%	\item 
\end{itemize}
\subsection{Scenarios}
\subsubsection{Scenario 1}
Blair The Witch had to take her magical broom to the mechanic for the annual revision but she needs to go shopping to refill her stockpile of frog?s tails. Her friend Mizune has told her about TAXINASECOND, so she decides to give it a try. After downloading it on her smartphone, she signs up compiling the registration form with her username, password, email and credit card data. Now she can complain about how slow car-based transports are!
