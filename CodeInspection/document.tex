...
%state theÂ namespace patternÂ and name of the classes	
...
Method names are meaningful, but 2 problems emerged: %TODO segnala solo i problemi -  vedi nota dopo
\begin{itemize}
...
Method variables are meaningful even if the use of names longer than 2 characters is suggested.\\ %TODO i suggerimenti sono da togliere, non bisogna cercare di fixare i problemi, sono segnalarli
%TODO se ci sono variabili con nomi da due lettere e non sono meaningful sono da segnalare - stesso per le costanti
...
\subsubsection{6}
The convection is respected, but the variable "clh" has a meaningless name, because an acronym is used, but as a 3 letter lowercase word, which can be confusing. It is suggested to rename the variable clHierarchy or classLoaderHierarchy to improve readability. %TODO ma allora questo non è da mettere prima nel punto 1?
\subsubsection{7}
...
\subsubsection{9}
line 518 uses tabs to indent.\\ %TODO non c'è lo stesso ignore sia in validationmapping che in registerbean?
\begin{lstlisting}
513	                        try {
...
519	                        }
\end{lstlisting}
%TODO questa parte è da togliere, bisogna solo segnalare i problemi e non cercare di rimuoverli -- It is proposed to remove the line or mark it as a ``TODO: check if it is adeguate to ignore this''
\subsection{Braces}
%TODO with sonarqube
%TODO queste le avevo già controllate io a mano per i nostri metodi e non ci sono problemi
\subsection{File Organization}
\subsubsection{12}
%TODO togliere i suggerimenti e segnalare solo il problema
A blank line is suggested before line 55 to divide imports of different domains.
Remove the two blank lines 216 217 in order to have coherence in spacing.
Remove line 299, it is not good to end a method with a blank line.
Remove line 342 and 373
\subsubsection{13}
%TODO e non ce n'è nessuna da segnalare che avrebbe senso splittata?
...
\subsubsection{14}
Only one line exceeds 120 chars and it is 121 chars long.
It is line 278 and since it is a comment and exceeds by only one character two solutions are proposed:\\ %TODO non sono da proporre soluzioni
\begin{itemize}
	\item Leave the line as it is, since splitting it would be a low-level break in the middle of a parenthesis
	\item split it before the opened parentheses
\end{itemize}
...
>,=.+
\end{verbatim}
%TODO non è il caso di fare vedere quali linee sono?
...
%TODO togliere il suggerimento
It is suggested to at least provide a line of comment or doc for each method.
\subsubsection{19}
%TODO da aggiungere la linea di codice e dire che mancano data e motivi per essere commentata
...
\begin{itemize}
	...
	\item logFine %TODO questo è il Logger di java normale, la doc c'è
\end{itemize}
The other methods are coherent with documentation.
\subsubsection{23} %TODO togliere i suggerimenti
As stated in the point above, three public methods are not documented, and even if 2 are due to interface implementation at least a line of documentation to explain what the particular method does should be added.
...
\subsubsection{30}%TODO almost?
Constructors are almost always called unless it is intentional to assign ``null'' to the variable. %TODO ma tipo quando mi ritorna un oggetto da un metodo di un altro oggetto è da considerare come una chiamata al costruttore?
\subsubsection{31}
...
\subsubsection{33}
Declarations appear at the beginning of blocks except for the ``event'' method, in which they are declared based on context in which they are used. Adding subscopes is suggested. %TODO togliere suggerimento
%ENDROBSECTION (this is used to automatic todo checks
\subsection{Method Calls}
\subsubsection{34}
%TODO qui manca la documentazione di un sacco di metodi, le scrivo che non si trova e che quindi non posso controllare? devo anche specificare di quali metodi manca la documentazione?
\subsubsection{35}
%TODO macosacazzovuoldire?
\subsubsection{36}
%TODO per properly intende che vengono usate per qualcosa e non rimangono inutilizzate?
\subsection{Arrays}
\subsubsection{37}
...
%null non si può confrontare con ``equals''
...
%hai ragione, ma è molto comune, che facciamo? vuoi mettere di essere più esplicativi o linkare a una pagina risolutiva?
\subsubsection{43}
...
\subsubsection{50}
%TODO uhm boh...
%Che non siano catchate eccezioni che non possono essere sollevate, o di non avere una eccezione più generica catchata prima di una più dettagliata (nel qual caso hai del codice non raggiungibile)
\subsubsection{51}
%TODO maledetta lei e la sua cazzo di documentazione
%Implicit type conversion è una rogna da checkare, mi sa che ci converrà caricare il progetto in eclipse e farlo fare a lui, se vuoi ne parliamo
...
%si che ce l'ha
\subsubsection{53}
%TODO se quando catcho un'eccezione la loggo e basta Ã¨ da ritenere un'azione appropriata?
%eh, se l'eccezione è del tipo ``connection reset'' o ``invalid file'' direi di si'
...
\subsection{Files} %TODO questo Ã¨ un boh generale T.T scusami T.T
