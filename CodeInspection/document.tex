...
519	                        }
\end{lstlisting}
It is proposed to remove the line or mark it as a ``TODO: check if it is adeguate to ignore this''
\subsection{Braces}
%TODO with sonarqube
...
\subsubsection{14}
Only one line exceedes 120 chars and it is 121 chars long.
It is line 278 and since it is a comment and exceedes by only one character two solutions are proposed:\\
\begin{itemize}
...
\subsubsection{15}
Line wrappings occour only after the following characters: 
\begin{verbatim}
>,=.+
\end{verbatim}
In lines 640 and 649 lines are broken before a parentheses in order to have a higher-level break so the convenction is respected.
\subsubsection{16}
Higher level breaks are used.
\subsubsection{17}
Allignment of new statements are correct.
\subsection{Comments}
\subsubsection{18}
...
\begin{itemize}
	...
	\item preDestroy (due to impelementation of PreDestroy)
	\item logFine
\end{itemize}
...
\paragraph{D}
Static variables are after the instance variables, so the convenction is not respected.
\paragraph{E}
...
\subsubsection{30}
Constructors are almost always called unless it is intentional to assign ``null'' to the variable.
\subsubsection{31}
...
\subsubsection{32}
Every variable is initialized where it is declared, and if it requires futher computation to be assigned with a value it is explicitly initialized with null, which sometimes is a valid value (see 31), and later computed and assigned.
\subsubsection{33}
Declarations appear at the beginning of blocks except fpr the ``event'' method, in which they are declared based on context in which they are used. Adding subscopes is suggested.
