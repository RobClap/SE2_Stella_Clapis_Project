	...
	\item \textit{Notifications sent to logged in users:} \\
		\begin{adjustbox}{width=1.2\textwidth}	
			\begin{tabular}{*{3}{c}}
				...
				%TODO devo aggiungere le notifiche di avvenuta cancellazione? c'è da fare un check di coerenza con tutte le notifiche nei sd
				\bottomrule
			\end{tabular}
		\end{adjustbox}	
	\item \textit{Notifications sent to taxi drivers:} \\
		\begin{adjustbox}{width=1.2\textwidth}	
			\begin{tabular}{*{3}{c}}
				...
				notifyDeletedRequest %TODO ma se facessimo solo sparire il messaggio della richiesta quando viene cancellata prima che faccia in tempo ad accettarla?
				\bottomrule
			\end{tabular}
		\end{adjustbox}	
\end{itemize}		
