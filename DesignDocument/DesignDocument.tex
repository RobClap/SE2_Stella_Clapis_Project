...
\footnote{See RASD's section 1.5.2}
\begin{figure}[H]
...
Some details:
\begin{itemize}
	\item HTTPS negotiations are omitted in order to preserve readability, but they would all be between the apps (Web or Mobile) and the httpHandler %TODO spiega cosa sono le negotiations
	...
	\begin{table}[H]
		\begin{adjustbox}{width=1.2\textwidth}
			\begin{tabular}{*{3}{c}}
				...
				%TODO dobbiamo mettere qualche metodo che ritorni il CAT perchè non ne abbiamo e poi metterlo nel sd apiusage...a meno che non gli facciamo tiraer fuori tutti i taxi e gli facciamo computare l'eta passandoli tutti<<-- io voto per questa
...
This interface will expose only one method, that will receive the messages from the server and will handle them according to their destination: 
	\begin{itemize}
		...
		\item ClientUI/DriverUI:\@ this will change the Activity shown with a new one (encoded in the message) that will show the text. %TODO troppo specifico per android
\end{itemize}
	...
	\item \textit{Notifications sent to logged in users:} \\
		\begin{adjustbox}{width=1.2\textwidth}	
			\begin{tabular}{*{3}{c}}
				...
				%TODO devo aggiungere le notifiche di avvenuta cancellazione?
				\bottomrule
			\end{tabular}
		\end{adjustbox}	
	\item \textit{Notifications sent to taxi drivers:} \\
		\begin{adjustbox}{width=1.2\textwidth}	
			\begin{tabular}{*{3}{c}}
				...
				notifyDeletedRequest %TODO ma se facessimo solo sparire il messaggio della richiesta quando viene cancellata prima che faccia in tempo ad accettarla? nel sd di cancel c'è un coso che mi fa abort ma è bruttino da vedere XD la parola abort non mi piace xD
				\bottomrule
			\end{tabular}
		\end{adjustbox}	
\end{itemize}		
...
For the Web Server it was decided to adopt a 3-tier subdivision: the untrusted internet, the DMZ and the internal network. %TODO qualche dettaglio zuccheroso in più? mi sembra un po poco
